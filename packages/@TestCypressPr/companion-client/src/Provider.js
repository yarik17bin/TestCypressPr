...
export default class Provider extends RequestClient {
  ...
  list(directory, opts) {
    return this.get(`${this.id}/list/${directory || ''}`, opts)
  }

  async logout(opts) {
    const response = await this.get(`${this.id}/logout`, opts)
    await this.removeAuthToken()
    return response
  }
  ...
  static initPlugin(plugin, opts, defaultOpts) {
    ...
    if (opts.serverUrl || opts.serverPattern) {
      throw new Error('`serverUrl` and `serverPattern` have been renamed to `companionUrl` and `companionAllowedHosts` respectively in the 0.30.5 release. Please consult the docs (for example, https://uppy.io/docs/instagram/ for the Instagram plugin) and use the updated opts.`')
    }
