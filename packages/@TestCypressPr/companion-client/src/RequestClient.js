...
export default class RequestClient {
  ...
  async get(path, opts = undefined) {
    // TODO: remove boolean support for opts that was added for backward compatibility.
    // eslint-disable-next-line no-param-reassign
    if (typeof opts === 'boolean') opts = { skipPostResponse: opts }
    return this.request({ ...opts, path })
  }

  async post(path, data, opts = undefined) {
    // TODO: remove boolean support for opts that was added for backward compatibility.
    // eslint-disable-next-line no-param-reassign
    if (typeof opts === 'boolean') opts = { skipPostResponse: opts }
    return this.request({ ...opts, path, method: 'POST', data })
  }

  async delete(path, data = undefined, opts) {
    // TODO: remove boolean support for opts that was added for backward compatibility.
    // eslint-disable-next-line no-param-reassign
    if (typeof opts === 'boolean') opts = { skipPostResponse: opts }
    return this.request({ ...opts, path, method: 'DELETE', data })
  }
   ...
   * @returns 
   */
  async uploadRemoteFile(file, reqBody, opts = {}) {
    try {
      const { signal, getQueue } = opts
  ...
  async #awaitRemoteFileUpload({ file, queue, signal }) {
    ...
    try {
      return await new Promise((resolve, reject) => {
        ...
        const generateWebsocket = async () => {
          if (socketAbortController) socketAbortController.abort()
          socketAbortController = new AbortController()

          const onFatalError = (err) => {
            // Remove the serverToken so that a new one will be generated for the retry.
